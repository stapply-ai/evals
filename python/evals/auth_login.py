...
async def main():
    ...
    try:
        ...
        go_to_url = "http://localhost:5173/eval/login"

        credentials = {"x_user": "test@example.com", "x_pass": "test1234"}
        ...
        agent = Agent(
            ...
            2. If there is a login form, fill it with the sensitive data. Never create a new account.
			3. Use the 'playwright_file_upload' action to upload the file at: {go_to_url}
			   - the file is ./resume.pdf
            4. For filling the form, not for the auth login but only for the form, use the following information:
            My name is Agent Stapply. My email is agent@stapply.ai.
			""",
            llm=llm,
            tools=tools,
            sensitive_data=credentials,
            browser_session=browser_session,
            use_vision=False, # Best practice: disable vision
            calculate_cost=True,
        )
        ...
        try:
            result_file = save_eval_result(
                eval_name="auth_login",
                model_name=llm.model,
                input_tokens=summary.total_tokens,
                output_tokens=summary.total_tokens,
                execution_time_seconds=execution_time,
                additional_data={
                    "task_description": "Browser automation for auth login using Playwright + Browser-Use",
